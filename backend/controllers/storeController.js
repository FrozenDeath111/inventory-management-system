...
const stockActivityModel = require("../models/stockActivityModel");
    ...
    if (role === 3) {
      ...
      for (index in storeData) {
        const product = await Product.findOne({
          _id: storeData[index].product_id,
        });
...
const showProduct = async (req, res) => {
  ...
  try {
    ...
    if (role === 3) {
      const store = await Store.find({ store_manager_id: _id }).select(
        "product_id"
      );
...
const updatePending = async (req, res) => {
    const { username } = req.user;
    const {_id, state, staff_username} = req.body;

    try {
        const { role } = await User.findOne({ username }).select("role");

        if(role===1){
            await Request.updateOne({_id}, {$set: {
                state,
            }})

            const data = await Request.findOne({_id});

            const product_id = await Product.findOne({name: data.product_name}).select("_id");
            
            await stockActivityModel.create({
            ...
            res.status(200).json({msg: "Action "+state+" Successful"});
        } else {
            res.status(401).json({ error: "Authorization Level Problem" });
        }
    } catch (error) {
